...
- **A button carries everything needed to answer it.** The navigation trail is encoded in `custom_id` as fixed-width 6-digit card ids, target last (`_switch_id`/`_parse_stack`) — never in a dict keyed by message id. Fixed width holds only while every corpus id is 6 digits, and `load_cards` refuses to serve a corpus that breaks it — a wider id desyncs every frame parsed after it, silently, so this fails at startup rather than on the next CI push. `MAX_FRAMES` derives the depth from the 100-char ceiling against the 8-char `switch-1` prefix — 15 — and drops the oldest frames past it. Label truncation is display-only: it must never touch `custom_id`, which the dispatch tables and the trail both read.
- A live button can outlive **the corpus** it names and **the encoding** that spelled it — a deploy leaves the previous release's buttons clickable until their tokens die. Both are expected failures, not bugs: look cards up through `_card()` and parse through `_parse_stack()`, which raise `CommandFailed`. Changing the `switch-` encoding without changing its 6-char dispatch prefix is what makes old buttons unreadable rather than merely wrong — so the prefix never changes (eng, T-011): an unknown one misses `COMPONENTS` and shows "Command error" instead of a `CommandFailed` the reader can act on. What a new encoding bumps instead is the version char in `SWITCH_PREFIX`, checked before width and digits — those two alone cannot tell one encoding from another that agrees on both, and the format that parses clean answers with the wrong cards.
- `_build_components` renders a stack **one frame short of `MAX_FRAMES`**. At the full ceiling `< Back` and a ruling link truncate to the same frames on a trail that ping-pongs between two cards, and a duplicate `custom_id` is a 400 on the whole message. Not hypothetical: the corpus holds 58 mutually-citing pairs.
