...
- **Fixed width holds only while every corpus id is 6 digits.** A wider id desyncs every frame
  parsed after it, silently. The bot refuses to start on such a corpus, and a test catches it in CI;
  both, because the test fires only on a push ([corpus.md](corpus.md#card-ids)).
