...
- `just release` = bump, tag, push, build, PyPI publish, then `gh release create` — that last step is what fires `deploy.yml`. A tag push alone does not create a release and so does not deploy.
