...
- **Prose budgets are the standing exception** (Lionel, T-014): they stay prose. A hook counting lines in `tickets/*.md` would police length, which is the proxy — density is the thing, and a ticket can be short and useless. The `≤150 words` advisor cap is the evidence that a small budget loaded early holds without enforcement. Do not re-file hook-enforced prose limits.
...
A sweep, not a quota (T-015). Each item is a question that may answer "no". A clean audit is the expected result — 16 of the 23 markers to date changed nothing — and an empty `harness-audit:` commit is a finished audit, not a skipped one. Report only findings you can point at; do not promote a suspicion to a finding to make the sweep look productive.
...
6. Standing constraints — rejections, "not until X" triggers: is each one written into the doc it governs, with its trigger? A live ticket is the legitimate interim home (T-013): the write happens when it ships or is dropped, so a constraint sitting in an open ticket is not a finding. One in a *deleted* ticket, or in neither, is — there is no side log, and a constraint that lives only in git history is lost.
