...
4. Docs are AI-first — agents are the readers and reviewers. Terse, structured, one fact one home. `refs/` is that home for durable facts: auto memory is machine-local scratch, never the record, and never a place a fact lives alone.
...
- Verify before done: `just quality` passes (a Stop hook blocks session end while uncommitted code fails it), plus the check `refs/eng/INDEX.md` §Workflow names for the surface you touched. Show the evidence — the command and what it returned — never an assertion that it works.
...
| `refs/shared/` | Project facts, decisions, plan format |
...
| `.claude/hooks/` | Gates: plan brief at start, quality + audit at session end |
